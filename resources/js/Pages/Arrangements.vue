...
const structuredData = computed(() => {
    return JSON.stringify({
        ...
        description:
            'Colecția noastră de aranjamente florale sofisticate create în Târgu Mureș.',
        numberOfItems: props.arrangements?.data?.length || 0,
        itemListElement: (props.arrangements?.data || []).map(
            (item, index) => ({
                '@type': 'ListItem',
                position: index + 1,
                item: {
...
<template>
    <Head>
        ...
        <meta
            name="description"
            content="Compoziții florale sofisticate concepute pentru a înnobila orice spațiu. Aranjamente deosebite pentru cadouri, decor interior sau momente speciale, create cu atenție la detalii."
        />
        <link rel="canonical" :href="route('arrangements')" />

        <meta
            property="og:title"
            content="Aranjamente Florale | Amaryllis Târgu Mureș"
        />
        <meta
            property="og:description"
            content="Compoziții florale sofisticate concepute pentru a înnobila orice spațiu."
        />
    ...
    <GuestLayout>
        <div class="space-y-8 px-4 md:space-y-12 md:px-0">
            ...
            >
                <div
                    v-for="arrangement in arrangements.data"
                    :key="arrangement.id"
            ...
            <!-- Empty State -->
            <div
                v-if="!arrangements.data || arrangements.data.length === 0"
